...
const CharacterEditorPanel = (props) => {
    ...
    /**
     * The CharacterEditorPanel contains an HTML canvas, which should not be
     * sized with CSS because any images on it would be stretched. Instead, we
     * get the dimensions of the parent container and fit the canvas to it. We
     * also listen for resize events and adjust the canvas size when the size of
    ...
    useEffect(() => {
        ...
        const handleResize = _debounce(fitToContainer, debounceWait)
